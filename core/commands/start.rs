...
pub fn run(matches: &ArgMatches) -> i32 {
    ...
    match fs::read_to_string(slang_rute) {
        Ok(slang_content) => {
            match serde_yaml::from_str::<Sflynlang>(&slang_content) {
                Ok(project_settings) => {
                    let main_rute = format!(
                        "{}/{}",
                        current_directory,
                        project_settings.get_main()
                    );
                    ...
                    match fs::read_to_string(&main_rute) {
                        Ok(main_content) => {
                            let file = sflynlang_parser::File::new(
                                project_settings.get_main(),
                                main_content,
                            );
